...
- CSVエクスポート `/api/export/csv`、Slack通知 `/api/notify/slack` のモックAPIを追加し、ダッシュボード/ハブのボタンからトースト＋簡易ログ（console）を出すようにした。`useActionLogger` を用意。
- `/api/log` を追加し、主要アクションをモックログ送信。`useDashboardData` に簡易キャッシュを持たせ、期間/セグメントごとに再利用。
- ダッシュボード/アドホック双方で SWR によるフェッチ・キャッシュ・再検証に切替（`useDashboardData`, `useAdhocData`）。フィルタ変更時にログ送信。
- Slack/CSV モックのレスポンス内容（メッセージ・ファイル名）をトーストに表示し、アクションログに記録。ページ訪問時に「最近使った」を更新。
